...
export default () => {
  ...
  add([
    text("SPACE to play", { size: 24 }),
    pos(k.width() * 0.5, k.height() * 0.7),
    origin("center"),
  ]);

  add([
    text("Turn on hardware acceleration for smoother gameplay", { size: 24 }),
    pos(k.width() * 0.5, k.height() * 0.8),
    origin("center"),
  ]);

  add([
    text("Created by Rafale Bacalla using KaboomJS, Node, and Express", {
      size: 22,
    }),
